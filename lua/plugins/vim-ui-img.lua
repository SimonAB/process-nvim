...
---Screen columns available from the cursor to the right window edge.
---@return integer
function M.cursor_peek_available_width()
	return vim.api.nvim_win_get_width(0) - vim.fn.wincol() + 1
end

---Clamp cursor-relative peek width to the window; nil when too narrow to show.
---@param width integer
---@param min_width integer|nil
---@return integer|nil
function M.clamp_cursor_peek_width(width, min_width)
	min_width = min_width or 20
	local available = M.cursor_peek_available_width()
	if available < min_width then
		return nil
	end
